<template>
  ...
  <div v-if="!authStore.isAutenticado" class="d-flex gap-3">
    <router-link to="/login" class="enter-icon-btn" title="Acessar login">
      <i class="bi bi-box-arrow-in-right display-6 text-white"></i>
    </router-link>
  </div>

  <!-- Se estiver autenticado → mostra botão de Dashboard -->
  <div v-else class="d-flex gap-3">
    <router-link to="/dashboard" class="enter-icon-btn" title="Ir para o painel">
      <i class="bi bi-speedometer2 display-6 text-white"></i>
    </router-link>
  </div>
